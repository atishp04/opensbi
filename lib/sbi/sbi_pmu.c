...
static int pmu_validate_fw_ctr(uint32_t cidx)
{
	uint32_t evt_idx_val;
	uint16_t evt_idx_type;
	uint16_t evt_idx_code;
	u32 hartid = current_hartid();

	evt_idx_val = active_events[hartid][cidx];
	evt_idx_type = get_cidx_type(evt_idx_val);
	evt_idx_code = get_cidx_code(evt_idx_val);

	if ((evt_idx_type != SBI_PMU_EVENT_TYPE_FW) ||
	    (evt_idx_code >= SBI_PMU_FW_MAX))
		return SBI_EINVAL;

	return evt_idx_code;
}

int sbi_pmu_read_fw_ctr(uint32_t cidx, uint64_t *cval)
{
	u32 hartid = current_hartid();
	struct sbi_pmu_fw_event fevent;
	int fw_evt_code;

	fw_evt_code = pmu_validate_fw_ctr(cidx);
	if (fw_evt_code < 0)
		return fw_evt_code;
...
/* Add a hw ctr read for completeness */
static int sbi_pmu_read_hw_ctr(uint32_t cidx, uint64_t *cval)
{
	...
	evt_idx_val = active_events[hartid][cidx];
	evt_idx_type = get_cidx_type(evt_idx_val);
	if (evt_idx_type > SBI_PMU_EVENT_TYPE_HW_RAW)
		return SBI_EINVAL;
#if __riscv_xlen == 32
	uint64_t temp, temph = 0;

	temp = csr_read_num(CSR_MCYCLE + cidx) << BITS_PER_LONG;
...
int sbi_pmu_read_ctr(uint32_t cidx, uint64_t *cval)
{
	u32 hartid = current_hartid();
	int total_ctrs = num_hw_ctrs + SBI_PMU_FW_CTR_MAX;

	if (cidx >= total_ctrs || (active_events[hartid][cidx] == SBI_PMU_EVENT_IDX_INVALID))
		return SBI_EINVAL;

	if (cidx < num_hw_ctrs)
		sbi_pmu_read_hw_ctr(cidx, cval);
	else
		sbi_pmu_read_fw_ctr(cidx, cval);
...
static int pmu_start_fw_ctr(uint32_t cidx, uint64_t ival)
{
	u32 hartid = current_hartid();
	struct sbi_pmu_fw_event *fevent;
	int fw_evt_code;
...
int sbi_pmu_start_ctr(uint32_t cidx, uint64_t ival)
{
	...
	if (cidx >= total_ctrs || (active_events[hartid][cidx] == SBI_PMU_EVENT_IDX_INVALID) ||
	    evt_idx_type >= SBI_PMU_EVENT_TYPE_MAX)
		return SBI_EINVAL;

	if (evt_idx_type == SBI_PMU_EVENT_TYPE_FW)
		return pmu_start_fw_ctr(cidx, ival);
	else
...
static int pmu_stop_fw_ctr(uint32_t cidx)
{
	u32 hartid = current_hartid();
	int fw_evt_code;
...
static int pmu_find_hw_ctr(unsigned long event_idx, uint64_t data,
			  ...
			  unsigned long flags)
{
	...
	if (event_type != SBI_PMU_EVENT_TYPE_FW) {
		ctr_idx = pmu_find_hw_ctr(event_idx, event_data, cidx_base, cidx_mask);
	} else {
...
int sbi_pmu_get_ctr_info(uint32_t cidx, unsigned long *ctr_info)
{
	...
	if (cidx < num_hw_ctrs) {
		...
		/* firmware counters are always 64 bit wide */
		cinfo.width = 63;
	}
